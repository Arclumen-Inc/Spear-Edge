...
def test_rid_wrapper_normalizes_minimal_backend_payload(tmp_path: Path):
    ...
    backend_script.write_text(
        "\n".join(
            [
                ...
                "a=p.parse_args()",
                "payload={",
                "  'fields': {'id':'RID-MIN-1','lat':32.7,'lon':-96.8},",
                "  'crc_pass': True,",
                "  'frame_count': 2",
                "}",
    ...
    env = os.environ.copy()
    env["SPEAR_RID_BACKEND_CMD"] = f"python {backend_script}"
    cmd = [
        "python",
        "scripts/rid_decode_wrapper.py",
        "--iq-path",
        str(iq_path),
        "--center-freq-hz",
        "2437000000",
    ...
    assert data["decoded_fields"]["uas_id"] == "RID-MIN-1"
