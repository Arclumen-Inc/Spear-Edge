#!/usr/bin/env python3
"""
DJI DroneID decode wrapper for SPEAR-Edge Phase 1B.

Optional backend delegation:
  - Set SPEAR_DJI_BACKEND_CMD to a command that accepts:
...
def _normalize_payload(raw: Dict[str, Any], args: argparse.Namespace) -> Dict[str, Any]:
    protocol = str(raw.get("protocol", "dji_droneid") or "dji_droneid")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="SPEAR DJI decode wrapper")
    ...
    backend_cmd = os.getenv("SPEAR_DJI_BACKEND_CMD", "").strip()
    if backend_cmd:
